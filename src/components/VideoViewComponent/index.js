...
onStateChange (e){
 ...
 onReady(){
   ...
   // let result = this.state.newarr.filter(o1 => parseInt(o1) === parseInt(elapsed_sec));
   // console.log("secondsss",parseInt(elapsed_sec),this.state.newarr)
    if(parseInt(elapsed_sec) === this.state.pausedtime){
   ...
   // alert(this.state.newarr.indexOf(this.state.pausedtime)-1)
    ...
    if(value === -1){
     ...
     this.setState({
      ...
      this._youTubeRef.current.seekTo(this.state.newarr[value],true);//(this.state.newarr[value]);
          this.setState({
              isPlaying: true,
              show:false,
...
/> */}
     ...
     height ={this.state.fullscreen ? "100%" : "50%"}
      ...
      />
        ...
        style={{ top:50,right:0,position:"absolute",padding:10,right:10}}>
          {this.state.fullscreen ? 
       ...
       <View style={[styles.absview,
        {top:this.state.fullscreen ? 360 : 340}]}>
